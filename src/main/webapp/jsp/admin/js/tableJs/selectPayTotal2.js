...
function reDrawPayTotal()
{
    ...
    var rechargeBitCoinArr = []; // 用于接收数据中的  充值比特币
    var rechargeCNYArr = [];     // 用于接收数据中的  充值人民币
        ...
        $.post("http://localhost:8080/bitCoin/manager/newUser.do?start="+startTime+"&end="+endTime,$("#dataForm").serialize(),function(data)
        {
            ...
            for(var i = 0; timestamp2 <= timestamp3; i++)
            {
                ...
                }else
                {
                    timerArr[j]=timerz;
                    timerTranArr[j] = getLocalTime(timerArr[j]).substr(0,17);
                    rechargeBitCoinArr[j] = data.data[i].rechargeBitCoin;        // 数据中的  充值比特币
                    rechargeCNYArr[j] = data.data[i].rechargeCNY;                // 数据中的  充值人民币
                }
                j=j+1;
        ...
        layui.use(['laypage', 'layer'], function()
        {
            ...
            var render = function(curr)
            {
                ...
                for(var i = (curr*nums - nums); i <= last; i++)
                {
                    ...
                    str += '<td>'+rechargeBitCoinArr[i]+'</td>';
                    str += '<td>'+rechargeCNYArr[i]+'</td></tr>';
                }
                return str;
            };
//        调用分页
            laypage({
                ...
                jump: function(obj)
                {
                    ...
                    tbody.innerHTML = '';
                    var text = "<th>时间</th> <th>充值总额(BTC)</th> <th>充值总金额(￥)</th>" + render(obj.curr);
