...
function reDrawIPView()
{
    ...
    var IPViewDataArr = [];  // 用于接收后台的 ip 浏览量数据
    ...
    startBut.onclick = function()
    {
        ...
        $.post("http://localhost:8080/bitCoin/manager/ipVisit.do?start="+startTime+"&end="+endTime,$("#dataForm").serialize(),function(data)
        {
            ...
            for(var i = 0; timestamp2 <= timestamp3; i++)
            {
                ...
                }else
                {
                    timerArr[j]=timerz;
                    timerTranArr[j] = getLocalTime(timerArr[j]).substr(0,17);
                    IPViewDataArr[j] = data.data[i].iPNumber;
                }
                j=j+1;
