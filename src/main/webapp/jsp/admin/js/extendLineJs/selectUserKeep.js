            ...
            for(var i = 0; timestamp2 <= timestamp3; i++)
            {
            	...
            	if(dataLen==0){
                    ...
                    }else
                    {
                        timerArr[j]=timerz;
                        timerTranArr[j] = getLocalTime(timerArr[j]).substr(0,9);
                        registerNumArr[j] = data.data[i].newUserNum;
                        ...
                        morrowKeepRateArr[j] = (morrowLoginArr[j] / registerNumArr[j]).toFixed(2);  // 次日留存率
                        threeKeepRateArr[j] = (threeLoginArr[j] / registerNumArr[j]).toFixed(2);  // 三日留存率
                        sevenKeepRateArr[j] = (sevenLoginArr[j] / registerNumArr[j]).toFixed(2);  // 七日留存率
                        monthKeepRateArr[j] = (monthLoginArr[j] / registerNumArr[j]).toFixed(2);  // 月留存率
                    }
                    j=j+1;
